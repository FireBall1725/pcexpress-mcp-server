...
def emit(target, env):
    ...
    if target in ("local", "docker"):
        ...
        print(
            "   mounted at PCEXPRESS_STATE_DIR — that PVC is what keeps the rotating token alive)."
        )
...
def main():
    ...
    if not cfg.CLIENT_SECRET:
        sys.exit(
            "No client secret available. Set PCEXPRESS_CLIENT_SECRET or bake one in pcid_config.py."
        )
    ...
    banner = ask(f"\nStore banner ({'/'.join(BANNERS)})", "zehrs").lower()
